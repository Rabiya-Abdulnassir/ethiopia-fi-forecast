...
def load_data():
    ...
    data_path = base_dir / "data" / "processed" / "ethiopia_fi_enriched_data.csv"
...
page = st.sidebar.selectbox(
    "Select Page",
    ["Overview", "Trends", "Forecasts", "Events Timeline", "Inclusion Projection"],
)
...
if page == "Overview":
    ...
    mm = df[df["indicator_code"] == "ACC_MM_ACCOUNT"]

    c1, c2, c3, c4 = st.columns(4)

    with c1:
        if len(acc) > 0:
            st.metric("Account Ownership", f"{acc['value_numeric'].max():.1f}%")

    with c2:
        if len(mm) > 0:
            st.metric("Mobile Money Accounts", f"{mm['value_numeric'].max():.1f}%")
...
# TRENDS
    ...
    indicator = st.selectbox("Choose Indicator", df["indicator_code"].dropna().unique())

    data = df[df["indicator_code"] == indicator]

    fig = px.line(data, x="year", y="value_numeric", markers=True, title=indicator)

    st.plotly_chart(fig, use_container_width=True)

    channels = df[
        df["indicator_code"].isin(
            ["ACC_MM_ACCOUNT", "USG_TELEBIRR_USERS", "USG_MPESA_USERS"]
        )
    ]
...
# FORECASTS
    ...
    forecast_file = base_dir / "reports" / "account_ownership_forecast.csv"
    ...
    if forecast_file.exists():
        ...
        fig = px.line(
            forecast, x="Year", y=scenario, markers=True, title=f"{scenario} Forecast"
        )
        ...
        st.download_button(
            "Download Forecast CSV", forecast.to_csv(index=False), "forecast.csv"
        )

    else:

        st.warning("Forecast file not found in reports folder.")
...
# INCLUSION PROJECTION
    ...
    st.header("Progress Toward 60% Inclusion Target")
    current = 49
    target = 60
    ...
    fig = go.Figure(
        go.Indicator(
            mode="gauge+number",
            value=current,
            title={"text": "Financial Inclusion (%)"},
